...
实际作画时，没必要涂黑整个图形，正如之前所说的：“你只需要知道元素在哪个集合就好了”。涂黑或涂灰更适合表示集合相交或互补的部分。
...
不，我们不能。因为序列也可以被看作一种容器，这样的话这个集合包含的唯一元素就是容器而不是序列里面若干个项了。_所以我们应当说集合 ${e_1,e_2,e_3,...}$ 包含了 $e_1,e_2,e_3,...$ 的所有项，而不是序列本身_。
